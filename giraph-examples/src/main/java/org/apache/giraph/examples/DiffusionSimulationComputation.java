...
public class DiffusionSimulationComputation extends BasicComputation<LongWritable, DiffusionVertexValue, NullWritable, IntWritable> {
	...
	public void compute(Vertex<LongWritable, DiffusionVertexValue, NullWritable> vertex, Iterable<IntWritable> msgs)
		...
		if(getSuperstep()==0) { //First superstep just to know the first label to analyze
			...
			boolean  byLabel = ((BooleanWritable)getAggregatedValue(DiffusionMasterCompute.byLabelOption)).get();
			int currentLabel = (int)((LongWritable)getAggregatedValue(DiffusionMasterCompute.currentLabel)).get();
			// aggregators must be analyzed after first superstep
			boolean timeToSwitch = ((BooleanWritable)getAggregatedValue(DiffusionMasterCompute.timeToSwitch)).get();
			if (timeToSwitch)//time to switch label?
				if(value.getLabel()<currentLabel)
					aggregate(DiffusionMasterCompute.nextLabel, new LongWritable(value.getLabel()));
				...
				if (byLabel) {
					if(!value.isVertexDead()) {		//Update the using probability, if not dead
						...
						if(activeNeighbors==value.getVertexThreshold())
							aggregate(DiffusionMasterCompute.hesitantVerticesAggregator,new LongWritable(1));
					}
					aggregateVerticesBasedOnProbability(value);
					if(value.rollActivationDice()) { //if this vertex is using the feature
						aggregate(DiffusionMasterCompute.usingVerticesAggregator, new LongWritable(1));
						sendMessageToAllEdges(vertex, new IntWritable(1));
					}	
				}else {//computation by min number
					if(!timeToSwitch) {				
						boolean justChanged = ((BooleanWritable)getAggregatedValue(DiffusionMasterCompute.justChangedTimeToSwitch)).get();
						...
						aggregate(DiffusionMasterCompute.potentialVerticesAggregator, new LongWritable(1));
					}
				}
			}
			//vertex.voteToHalt();
	...
	private void aggregateVerticesBasedOnProbability(DiffusionVertexValue value) {
		if(value.isVertexConvinced())
			aggregate(DiffusionMasterCompute.convincedVerticesAggregator, new LongWritable(1));
		//LOG.info("I'm with a probability " + value.getCurrentActivationProbability());
		if(value.isVertexDead()) { //Dead aggregator update
			aggregate(DiffusionMasterCompute.deadVerticesAggregator, new LongWritable(1));
		}
		if(value.isAlmostConvinced()){
			aggregate(DiffusionMasterCompute.almostConvincedVerticesAggregator,new LongWritable(1));
		}
		aggregate(DiffusionMasterCompute.invitedVerticesAggregator, new LongWritable(1));
