...
public class TestFixedCapacityLongByteMinHeap {
  @Test
  public void testHeap() {
    int heapCapacity = 5;
    FixedCapacityLongByteMinHeap heap =
        new FixedCapacityLongByteMinHeap(heapCapacity);
    long[] keys = new long[]{0, 1, 0, 10, 20, 0, 3, 4};
    byte[] values = new byte[]{
        (byte) 0, (byte) 1, (byte) 2, (byte) 2, (byte) 2,
        (byte) 3, (byte) 3, (byte) 4};
    ...
    for (int i = keys.length - heapCapacity; i < keys.length; i++) {
      ...
      Assert.assertEquals(heap.getMinValue(), values[i]);
      heap.removeMin();
      heapCapacity--;
    }
