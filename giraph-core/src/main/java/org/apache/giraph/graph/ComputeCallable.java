...
public class ComputeCallable<I extends WritableComparable, V extends Writable,
  ...
  public Collection<PartitionStats> call() {
    ...
    while (true) {
      ...
      try {
        System.out.println("outgoing: " + serviceWorker.getConfiguration().getOutgoingMessageClasses().ignoreExistingVertices() + ", incoming: " + serviceWorker.getConfiguration().getIncomingMessageClasses().ignoreExistingVertices());
        ...
        PartitionStats partitionStats = computePartition(
            ...
            serviceWorker.getConfiguration().getIncomingMessageClasses().ignoreExistingVertices());
  ...
  private PartitionStats computePartition(
    ...
    synchronized (partition) {
      if (ignoreExistingVertices) {
        Iterable<I> destinations = messageStore.getPartitionDestinationVertices(partition.getId());
