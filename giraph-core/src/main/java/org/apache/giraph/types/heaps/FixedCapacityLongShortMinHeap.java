...
import org.apache.giraph.function.primitive.pairs.LongShortConsumer;
import org.apache.giraph.function.primitive.pairs.LongShortPredicate;

// AUTO-GENERATED class via class:
// org.apache.giraph.generate.GeneratePrimitiveClasses

/**
 * Min heap which holds (long key, short value) pairs with
 ...
 */
public class FixedCapacityLongShortMinHeap
    implements Long2ShortMapEntryIterable {
  /** Keys in the heap */
   ...
   */
  public FixedCapacityLongShortMinHeap(int capacity) {
   ...
   * @param value Value
   */
  public void add(long key, short value) {
   ...
   */
  protected int compare(long key1, short value1,
      long key2, short value2) {
    int t = Short.compare(value1, value2);
   ...
   */
  public static void write(FixedCapacityLongShortMinHeap heap,
      DataOutput out) throws IOException {
    out.writeInt(heap.capacity);
    out.writeInt(heap.size);
    for (int i = 0; i < heap.size(); i++) {
      out.writeLong(heap.keys[i]);
      out.writeShort(heap.values[i]);
    }
  }
   ...
   * @param in   Data input
   * @return FixedCapacityLongShortMinHeap deserialized from data input.
   * @throws IOException
   */
  public static FixedCapacityLongShortMinHeap read(
      FixedCapacityLongShortMinHeap heap, DataInput in)
      throws IOException {
    int capacity = in.readInt();
    if (heap == null || heap.capacity != capacity) {
      heap = new FixedCapacityLongShortMinHeap(capacity);
    } else {
      heap.clear();
    }
   ...
   */
  private int removeRootAndFindPosition(long key, short value) {
    int position = 0;
    while (position < size) {
      // Find the left child
   ...
   */
  public void forEachLongShort(LongShortConsumer f) {
   ...
   */
  public boolean forEachWhileLongShort(LongShortPredicate f) {
    for (int i = 0; i < size(); ++i) {
  ...
  /** Iterator for FixedCapacityLongShortMinHeap */
  private class IteratorImpl implements ObjectIterator<Long2ShortMap.Entry> {
    /** Reusable entry */
    ...
    public Long2ShortMap.Entry next() {
      ...
      entry.setShortValue(values[index]);
      return entry;
    }
  ...
  private static class MutableEntry extends AbstractLong2ShortMap.BasicEntry {
    /** Default constructor */
